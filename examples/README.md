...
### 1. マッチ選択ガイド（match_selection_guide.py）

**説明:**
複数のマッチ候補が見つかった場合に、どのマッチを選択すべきか判断するためのデモンストレーションです。マッチの可視化画像を生成し、選択方法を説明します。

**実行方法:**
```bash
cd examples
python match_selection_guide.py
```

**処理内容:**
1. リガンドとプローブ分子をロード
2. 部分構造探索を実行してマッチ候補を検出
3. 各マッチを可視化したPNG画像を生成
4. マッチ選択方法を表示

**主な機能:**
- 複数マッチの可視化
- match_indexパラメータの使用方法説明
- インタラクティブな選択ガイド

**出力:**
```
test_output/match_selection_guide.png  # マッチ可視化画像
```
...
### 2. マッチ可視化の例（visualize_matches_example.py）

**説明:**
部分構造マッチの可視化機能のサンプルコードです。A01プローブを使用した複数マッチの可視化方法を示します。

**実行方法:**
```bash
cd examples
python visualize_matches_example.py
```

**処理内容:**
1. リガンドとA01プローブをロード
2. 部分構造探索を実行
3. マッチ候補を可視化したPNG画像を生成
4. 結果のサマリーを表示

**主な機能:**
- visualize_multiple_matches関数の使用例
- 複数マッチの2D構造可視化
- ハイライト表示による視覚的な区別

**出力:**
```
test_output/substructure_matches_A01_test.png
```
...
### 3. 統合アライメント（integrated_alignment.py）⭐ 推奨
